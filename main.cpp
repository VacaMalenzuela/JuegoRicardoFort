	...
	while (true){
		...
		switch (page_number){
			case 0: {// jugar
				...
				puntaje.setCharacterSize(45);
				vidas.setPosition(810,55);
				vidas.setCharacterSize(45);
				level.setPosition(810, 105);
				level.setCharacterSize(45);
				puntMax.setPosition(810,315);
				...
				while (window_play.isOpen()){
					...
					if(comienzo){
						...
						for (int i = 0; i < ladrillos; i++){
							if(p.isCollision(l[i]) && !l[i].isRoto()){
								l[i].roto();
								//l[i].desaparece();
								p.rebote();
								puntos++;
						...
						if(p.isCollision(b)){
							p.rebote(b.getBounds().top);

						}

						if(pelota == false && vida > 1){
							vida--;
							canal.play();
							p.respawn();
							b.respawn();
						...
						if (ladrillos_rotos == ladrillos){
							...
							for (int i = 0; i < ladrillos; i++){
								l[i].desromper(); // vuelven a armarse la pared
							}
